...
//Time complexity: This runs in O(n) time because we only search for consecutive numbers if it is
//the FIRST in the sequence, this is done by checking is the number-1 does not exist in the HashSet.
//Space complexity: This is O(n) because we need to keep a HashSet of all the numbers.
class Solution {
    public int longestConsecutive(int[] nums) {
        ...
        for(int num : set){//for some reason, itterating over the HashSet does NOT cause a timeout, inlike the array... IDK WHY.
